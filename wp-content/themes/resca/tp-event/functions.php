...
if (!function_exists('thim_event_tabs')) {
	...
	function thim_event_tabs(){
		...
		$tab = isset($_GET['tab']) ? $_GET['tab'] : 'happening';
		...
		<div class="thim-event-tabs">
			...
			<ul class="nav nav-tabs">
				...
				<li class="tab happening <?php echo esc_attr($happening_active); ?>" data-tab="happening">
					<a href="#happening" aria-controls="happening" data-toggle="tab"><i class="fa fa-bookmark"></i><?php esc_html_e('Happening', 'thim'); ?></a>
				</li>
				<?php endif; ?>
				
				<?php if ($count['tp-event-upcoming'] != 0): ?>
				<li class="tab upcoming <?php echo esc_attr($upcoming_active); ?>" data-tab="upcoming">
					<a href="#upcoming" aria-controls="upcoming" data-toggle="tab"><i class="fa fa-cube"></i><?php esc_html_e('Upcoming', 'thim'); ?></a>
				</li>
				<?php endif; ?>

				<?php if ($count['tp-event-expired'] != 0): ?>
				<li class="tab expired <?php echo esc_attr($expired_active); ?>" data-tab="expired">
					<a href="#expired" aria-controls="expired" data-toggle="tab"><i class="fa fa-user"></i><?php esc_html_e('Expired', 'thim'); ?></a>
				</li>
				<?php endif; ?>
...
if ( !function_exists( 'thim_event_paging_nav' ) ) :
	...
	function thim_event_paging_nav($wp_query,$tab) {
		...
		$links = paginate_links( array(
			...
			'prev_text' => esc_html__( 'Prev', 'thim' ),
			'next_text' => esc_html__( 'Next', 'thim' ),
			'type'      => 'list'
		) );
