...
class WonderPlugin_Gridgallery_Update {
	...
	function __construct($controller) {
		...
		$this->product = 'gridgallery';
		$this->slug = 'wonderplugin-gridgallery';
		$this->plugin = WONDERPLUGIN_GRIDGALLERY_PLUGIN;
		...
		if( $pagenow === 'update-core.php' && !empty($info))
		{
			if ( !isset($info->key_status) || $info->key_status != 'valid')
			{
				...
				else
				{
					...
					{
						echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: To update the plugin, please <a href="'. admin_url('admin.php?page=wonderplugin_gridgallery_register') . '">enter your license key</a>, otherwise, you will get the <strong>Update package not available</strong> error.</div>';
					}
				}
			}
		}
	...
	function show_plugin_update_message( $plugin_data, $r ) {
		...
		else
		{
			if (WONDERPLUGIN_GRIDGALLERY_VERSION_TYPE == 'F')
			{
				echo '<br>To get automatic update, please <a href="https://www.wonderplugin.com/order/?product=gridgallery" target="_blank">upgrade to a commercial license</a>.';
			}
			else
			{
				echo '<br>To get automatic update, please <a href="'. admin_url('admin.php?page=wonderplugin_gridgallery_register') . '">enter your license key</a> or <a href="https://www.wonderplugin.com/register-faq/#manualupdate" target="_blank">update the plugin manually</a>.';
			}
		}
	}
}