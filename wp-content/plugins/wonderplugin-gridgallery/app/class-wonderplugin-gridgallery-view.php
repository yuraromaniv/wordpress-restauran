...
class WonderPlugin_Gridgallery_View {
	...
	function add_metaboxes() {
		add_meta_box('overview_features', __('WonderPlugin Portfolio Grid Gallery Features', 'wonderplugin_gridgallery'), array($this, 'show_features'), 'wonderplugin_gridgallery_overview', 'features', '');
	...
	function print_overview() {
		...
		<h2><?php echo __( 'WonderPlugin Portfolio Grid Gallery', 'wonderplugin_gridgallery' ) . ( (WONDERPLUGIN_GRIDGALLERY_VERSION_TYPE == "C") ? " Commercial Version" : " Free Version") . " " . WONDERPLUGIN_GRIDGALLERY_VERSION; ?> </h2>
		 
		<div id="welcome-panel" class="welcome-panel">
			<div class="welcome-panel-content">
				<h3>WordPress Portfolio Grid Gallery Plugin</h3>
	...
	function print_edit_settings() {
		...
		if ( isset($_POST['save-gridgallery-options']) && check_admin_referer('wonderplugin-gridgallery', 'wonderplugin-gridgallery-settings') )
		{		
		...
		$displaytitleineditor = $settings['displaytitleineditor'];
        ...
        <?php wp_nonce_field('wonderplugin-gridgallery', 'wonderplugin-gridgallery-settings'); ?>
		...
		<tr>
			<th>Gallery editor</th>
			<td><label><input name='displaytitleineditor' type='checkbox' id='displaytitleineditor' <?php echo ($displaytitleineditor == 1) ? 'checked' : ''; ?> /> Display the title in step 1, the images and videos list</label>
			</td>
		</tr>
		
		<tr>
			<th></th>
			<td><label><input name='usepostsave' type='checkbox' id='usepostsave' <?php echo ($usepostsave == 1) ? 'checked' : ''; ?> /> Use Post method to save the gallery</label>
			</td>
		</tr>
	...
	function print_register() {
		...
		if (isset($_POST['save-gridgallery-license']) && check_admin_referer('wonderplugin-gridgallery', 'wonderplugin-gridgallery-register') )
		{		
		...
		else if (isset($_POST['deregister-gridgallery-license']) && check_admin_referer('wonderplugin-gridgallery', 'wonderplugin-gridgallery-register') )
		{	
		...
		?> <div style="padding-left:10px;padding-top:12px;"> <?php
			if (empty($key)) { ?>
				<form method="post">
				<?php wp_nonce_field('wonderplugin-gridgallery', 'wonderplugin-gridgallery-register'); ?>
				<table class="form-table">
				<tr>
			...
			<?php } else { ?>
				<form method="post">
				<?php wp_nonce_field('wonderplugin-gridgallery', 'wonderplugin-gridgallery-register'); ?>
				<p>You have entered your license key and this domain has been successfully registered. &nbsp;&nbsp;<input name="wonderplugin-gridgallery-key" type="hidden" id="wonderplugin-gridgallery-key" value="<?php echo esc_html($key); ?>" class="regular-text" /><input type="submit" name="deregister-gridgallery-license" id="deregister-gridgallery-license" class="button button-primary" value="Deregister Your License Key"  /></p>
				</form>
	...
	function print_item()
	{
		...
		<div class="updated"><p style="text-align:center;">  <?php _e( 'To embed the gridgallery into your page, use shortcode: ', 'wonderplugin_gridgallery' ); ?> <?php echo esc_attr('[wonderplugin_gridgallery id=' . $_REQUEST['itemid'] . ']'); ?></p></div>

		<div class="updated"><p style="text-align:center;">  <?php _e( 'To embed the gridgallery into your template, use php code: ', 'wonderplugin_gridgallery' ); ?> <?php echo esc_attr('<?php echo do_shortcode(\'[wonderplugin_gridgallery id=' . $_REQUEST['itemid'] . ']\'); ?>'); ?></p></div>
		
		<?php
		if (WONDERPLUGIN_GRIDGALLERY_VERSION_TYPE !== "C")
			echo '<div class="updated"><p style="text-align:center;">To remove the Free Version watermark, please <a href="https://www.wonderplugin.com/order/?product=gridgallery" target="_blank">Upgrade to Commercial Version</a>.</p></div>';
	...
	function print_add_new() {
		...
		if ( !empty($_POST['wonderplugin-gridgallery-save-item-post-value']) && !empty($_POST['wonderplugin-gridgallery-save-item-post']) && check_admin_referer('wonderplugin-gridgallery', 'wonderplugin-gridgallery-saveform') )
		{
	...
	function save_item_post($item_post) {
		...
		?>
			...
			if (isset($ret['success']) && $ret['success'] && isset($ret['id']) && $ret['id'] >= 0) 
			{
				...
				echo "<div class='updated'><p>To embed the gallery into your page or post, use shortcode:  [wonderplugin_gridgallery id=" . $ret['id'] . "]</p></div>";
				echo "<div class='updated'><p>To embed the gallery into your template, use php code:  &lt;?php echo do_shortcode('[wonderplugin_gridgallery id=" . $ret['id'] . "]'); ?&gt;</p></div>"; 
			}
			else
			{
				echo "<h2>WonderPlugin Portfolio Grid Gallery</h2>";
	...
	function import_export()
	{
		?>
			<div class="wrap">
				<div id="icon-wonderplugin-gridgallery" class="icon32">
					<br />
				</div>

				<h2>
					<?php _e( 'Import/Export', 'wonderplugin_gridgallery' ); ?>
				</h2>
				...
				<ul class="wonderplugin-tab-buttons-horizontal"
					...
					<li
						...
						<?php _e( 'Import', 'wonderplugin_gridgallery' ); ?></li>
					<li class="wonderplugin-tab-button-horizontal"><span
						class="dashicons dashicons-upload" style="margin-right: 8px;"></span>
						<?php _e( 'Export', 'wonderplugin_gridgallery' ); ?></li>
						
					<li class="wonderplugin-tab-button-horizontal"><span class="dashicons dashicons-search" style="margin-right:8px;"></span><?php _e( 'Search and Replace', 'wonderplugin_gridgallery' ); ?></li>
				</ul>
				...
				<ul class="wonderplugin-tabs-horizontal"
					...
					<li
						...
						<?php 
						if (isset($_POST['wp-import']) && isset($_FILES['importxml']) && check_admin_referer('wonderplugin-gridgallery', 'wonderplugin-gridgallery-import'))
							$import_return = $this->controller->import_gridgallery($_POST, $_FILES);
						?>

						<form method="post" enctype="multipart/form-data">
							<?php wp_nonce_field('wonderplugin-gridgallery', 'wonderplugin-gridgallery-import'); ?>
							<?php 
					...
					if (empty($data)) {
						...
						<form method="post"
							action="<?php echo admin_url('admin-post.php?action=wonderplugin_gridgallery_export'); ?>">
							<?php wp_nonce_field('wonderplugin-gridgallery', 'wonderplugin-gridgallery-export'); ?>

							<p>
								<label><input type="radio" name="allgridgallery" value=1 checked>Export
									all galleries</label>
							</p>
							<p>
								<label><input type="radio" name="allgridgallery" value=0>Select a
									gallery: </label> <select name="gridgalleryid">
					...
					<?php 
		        	if (empty($data)) {
		        		echo '<p>No grid gallery found!</p>';
		        	} else {
		        	?>
		        	<h2>Search and Replace</h2>
					<form method="post">
		        	<?php wp_nonce_field('wonderplugin-gridgallery', 'wonderplugin-gridgallery-search-replace'); ?>
		        	<?php
		        	if (isset($_POST['wp-search-replace-submit']) && check_admin_referer('wonderplugin-gridgallery', 'wonderplugin-gridgallery-search-replace'))
		        	...
		        	?>
		        	<p><label><input type="radio" name="allitems" value=1 checked>Apply to all grid galleries</label></p>
		        	<p><label><input type="radio" name="allitems" value=0>Select a grid gallery: </label>
		        	<select name="itemid">
