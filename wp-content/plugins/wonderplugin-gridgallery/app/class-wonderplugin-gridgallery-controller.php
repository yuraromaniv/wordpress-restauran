...
require_once 'class-wonderplugin-gridgallery-update.php';
...
class WonderPlugin_Gridgallery_Controller {
	...
	function __construct() {
		...
		$this->update = new WonderPlugin_Gridgallery_Update($this);
	...
	function generate_body_code($id, $has_wrapper, $datatags = null) {
		...
		return $this->model->generate_body_code($id, $has_wrapper, $datatags);
	}
	...
	function init() {
		...
		$engine = array("WordPress Portfolio Plugin", "WordPress Portfolio");
	...
	{
		return $this->model->import_gridgallery($post, $files);
	}
	
	function export_gridgallery() {
	
		return $this->model->export_gridgallery();
	}
}